...
def _detect_lines(gray):
    ...
    """
    minLineLength = 100 #filtered out the shorter lines
    maxLineGap = 20
    # Detect all lines
        ...
        if mask[i]:
            for j, s_line in enumerate(horizontal_lines[i + 1:]):
                if mask[i + j + 1]:
                    ...
                    x3, y3, x4, y4 = s_line
                    dy = abs(y3 - y2)
                    if dy < 20: #The sensitivity of horizontal line merging can be adjusted
        ...
        if mask[i]:
            for j, s_line in enumerate(vertical_lines[i + 1:]):
                if mask[i + j + 1]:
                    ...
                    dx = abs(xi - xj)
                    if dx < 20:  #The sensitivity of vertical line merging can be adjusted
...
def _find_homography(horizontal_lines, vertical_lines,outer,g_or_c):
    ...
    for horizontal_pair in list(combinations(horizontal_lines, 2)):
        y1_max = max(horizontal_pair[0][1], horizontal_pair[0][3])
        y2_max = max(horizontal_pair[1][1], horizontal_pair[1][3])

        if ((y1_max <= mid_height and y2_max >= mid_height) or (y1_max >= mid_height and y2_max <= mid_height)):
            h1, h2 = horizontal_pair
        else:
            continue
        for vertical_pair in list(combinations(vertical_lines, 2)):
            # 假设 vertical_pair 是一个元组或列表，包含两个元组/列表，每个代表一个坐标对
            x1_max = max(vertical_pair[0][0], vertical_pair[0][2])
            x2_max = max(vertical_pair[1][0], vertical_pair[1][2])
            # 检查两个x坐标的最大值是否一边在mid_width左侧，另一边在右侧
            if ((x1_max <= mid_width and x2_max >= mid_width) or (x1_max >= mid_width and x2_max <= mid_width)):
                v1, v2 = vertical_pair
            else:
                continue
...
if __name__ == '__main__':
    ...
    for image_file in image_files:
        ...
        if img is None:
            ...
            horizontal_base_line  = [(0, int(v_height * 1/ 2), v_width, int(v_height * 1/ 2))]
            vertical_base_line = [(int(v_width / 2), 0, int(v_width / 2), v_height)]

            # 使用 draw_lines 绘制基准线
            # draw_lines(color_image, base_line, color=(0, 255, 0), thickness=2)  # 用绿色绘制水平线
            draw_lines(color_image, horizontal_base_line, color=(0, 255, 0), thickness=2)  # 用绿色绘制水平线
            draw_lines(color_image, vertical_base_line, color=(0, 255, 0), thickness=2)  # 用绿色绘制水平线

            # draw_lines(color_image, [(1593, 375, 1948, 1156)], color=(0, 255, 0), thickness=4)  # 用绿色绘制水平线
            # draw_lines(color_image, [(1078, 374, 691, 1166)], color=(0, 255, 0), thickness=4)  # 用绿色绘制水平线
